            ...
            function( item ) {
                ...
                var data =
                    {
                        ...
                        targetdelivery: ko.observable( item.ProposedStartDate ? moment(item.ProposedStartDate).format('MM/DD/YYYY') : '' ),
                        ...
                        cost: ko.observable( item.BusinessFTE ? item.BusinessFTE : '' ),
                        status: ko.observable( item.Id ? item.Id : '' ),
                        ...
                        alignment: ko.observable( item.Alignment ? item.Alignment.Title : null ),
                        group: ko.observable( item.Group ? item.Group.Title : null ),
                        primaryPM: ko.observable( item.PrimaryPM ? item.PrimaryPM.DisplayName : null ),
                        impactOnObjective: ko.observable( item.ImpactOnObjective ? item.ImpactOnObjective.replace(/(<([^>]+)>)/ig, "") : '' ),
                    };
                ...
                data.overAllHealth =
                    ko.computed(function() {
                        ...
                        return 'S - ' + SKPI + ', P - ' + PKPI + ', C - ' + CKPIO;
                    });

                data.Pcomplete =
                    ko.computed(function() {
                        var ProjectLevelTask =  ko.utils.arrayFilter(self.TaskDS, function(task) {
                            return task.TopLevel == true && task.Project.ItemId == data.id();
                        });
